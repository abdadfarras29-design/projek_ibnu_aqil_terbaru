...
<body>
        ...
        <main class="dashboard-content">
                            ...
                            <div style="display:grid;grid-template-columns:1fr 1fr;gap:1.5rem;">
                                ...
                                <div class="form-group">
                                    <label style="font-weight:600;display:block;margin-bottom:.5rem;">Kategori <span
                                            style="color:red;">*</span></label>
                                    <select id="beritaKategori" class="form-control" required
                                        style="width:100%;padding:.75rem 1rem;border:1.5px solid #d1d5db;border-radius:8px;font-size:1rem;background:#fff;">
            ...
            <div id="galeri" class="tab-content">
                <div class="page-header">
                    ...
                    <button class="btn-primary" onclick="bukaFormGaleri('tambah')" id="btnTambahGaleri">+ Tambah
                        Foto</button>
                </div>
                ...
                <div id="formGaleriWrap" style="display:none;">
                    <div class="section-card" style="border-left:4px solid #3b82f6;">
                        <h2 id="formGaleriTitle" style="margin-bottom:1.5rem;">Tambah Foto Baru</h2>
                        <form id="formGaleri" enctype="multipart/form-data">
                            <input type="hidden" id="galeriId" value="">
                            <div style="display:grid;grid-template-columns:1fr 1fr;gap:1.5rem;">
                                <div class="form-group" style="grid-column:1/-1;">
                                    <label style="font-weight:600;display:block;margin-bottom:.5rem;">Judul <span
                                            style="color:red;">*</span></label>
                                    <input type="text" id="galeriJudul" class="form-control"
                                        placeholder="Judul foto/kegiatan..." required
                                        style="width:100%;padding:.75rem 1rem;border:1.5px solid #d1d5db;border-radius:8px;font-size:1rem;">
                                </div>
                                <div class="form-group" style="grid-column:1/-1;">
                                    <label style="font-weight:600;display:block;margin-bottom:.5rem;">Foto <span
                                            style="color:red;">*</span></label>
                                    <div style="display:flex;align-items:center;gap:1rem;flex-wrap:wrap;">
                                        <input type="file" id="galeriFotoFile" accept="image/*"
                                            style="flex:1;padding:.6rem;border:1.5px dashed #d1d5db;border-radius:8px;"
                                            onchange="previewGaleriFoto(this)">
                                        <span style="color:#6b7280;font-size:.85rem;">atau URL</span>
                                        <input type="text" id="galeriFotoUrl" class="form-control"
                                            placeholder="URL eksternal..."
                                            style="flex:2;padding:.75rem 1rem;border:1.5px solid #d1d5db;border-radius:8px;font-size:1rem;">
                                    </div>
                                    <div id="galeriFotoPreviewWrap" style="margin-top:.75rem;display:none;">
                                        <img id="galeriFotoPreview" src="" alt="Preview"
                                            style="max-height:160px;border-radius:10px;object-fit:cover;border:2px solid #e5e7eb;">
                                        <button type="button" onclick="hapusGaleriFotoPreview()"
                                            style="margin-left:.75rem;background:#ef4444;color:#fff;border:none;padding:.35rem .8rem;border-radius:6px;cursor:pointer;font-size:.85rem;">✕
                                            Hapus</button>
                                    </div>
                                </div>
                                <div class="form-group" style="grid-column:1/-1;">
                                    <label style="font-weight:600;display:block;margin-bottom:.5rem;">Keterangan <small
                                            style="color:#6b7280;">(Opsional)</small></label>
                                    <input type="text" id="galeriKeterangan" class="form-control"
                                ...
                                <div class="form-group" style="grid-column:1/-1;">
                                    ...
                                    <textarea id="galeriDeskripsi" class="form-control" rows="3"
                                        placeholder="Deskripsi foto..."
                                        style="width:100%;padding:.75rem 1rem;border:1.5px solid #d1d5db;border-radius:8px;font-size:1rem;resize:vertical;"></textarea>
                                </div>
                            </div>
                            <div style="display:flex;gap:1rem;margin-top:1.5rem;">
                                <button type="submit" class="btn-primary" id="btnSimpanGaleri">💾 Simpan Foto</button>
                                <button type="button" class="btn-secondary" onclick="tutupFormGaleri()">Batal</button>
                            </div>
                        </form>
                    </div>
                </div>
                    ...
                    <div class="table-responsive">
                        <table class="data-table">
                            ...
                            <tbody id="tabelGaleri">
                                <tr>
                                    <td colspan="6" style="text-align:center;padding:2rem;color:#9ca3af;">⏳ Memuat
                                        data...</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                    <div id="galeriPagination" style="margin-top:1rem;display:flex;gap:.5rem;justify-content:flex-end;">
                    </div>
                </div>
            </div>

            <!-- Modal Detail Galeri -->
            <div id="modalDetailGaleri"
                ...
                <div
                    style="background:#fff;margin:5% auto;border-radius:12px;width:90%;max-width:800px;box-shadow:0 25px 60px rgba(0,0,0,0.5);overflow:hidden;animation:fadeInModal .25s ease;">
                    <span onclick="document.getElementById('modalDetailGaleri').style.display='none'"
                    ...
                    <img id="modalGaleriFoto" src="" alt="Foto"
                        style="width:100%;max-height:500px;object-fit:contain;background:#111;display:block;">
                    <div style="padding:1.5rem 2rem;">
                        <h2 id="modalGaleriJudul" style="font-size:1.5rem;color:#1f2937;margin-bottom:.5rem;"></h2>
                        <p id="modalGaleriKeterangan"
                            style="font-size:.9rem;color:#6b7280;font-weight:600;margin-bottom:1rem;"></p>
                        <p id="modalGaleriDeskripsi" style="color:#4b5563;line-height:1.6;"></p>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        let semuaGaleri = [];
        let halamanGaleri = 1;
        const perHalamanGaleri = 8;
        let uploadedGaleriPath = '';

        async function muatGaleri() {
            const tbody = document.getElementById('tabelGaleri');
            tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;padding:2rem;color:#9ca3af;">⏳ Memuat...</td></tr>';
            try {
                const res = await fetch('galery-api.php?action=list');
                const json = await res.json();
                if (!json.success) throw new Error(json.message);
                semuaGaleri = json.data;
                halamanGaleri = 1;
                renderTabelGaleri();
            } catch (e) {
                tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;padding:2rem;color:#ef4444;">❌ Gagal memuat: ' + e.message + '</td></tr>';
            }
        }

        function renderTabelGaleri() {
            const tbody = document.getElementById('tabelGaleri');
            const start = (halamanGaleri - 1) * perHalamanGaleri;
            const slice = semuaGaleri.slice(start, start + perHalamanGaleri);

            if (semuaGaleri.length === 0) {
                tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;padding:2rem;color:#9ca3af;">📭 Belum ada foto di galeri.</td></tr>';
                document.getElementById('galeriPagination').innerHTML = '';
                return;
            }

            tbody.innerHTML = slice.map((g, i) => {
                const no = start + i + 1;
                const foto = g.foto ? `<img src="${escHtml(g.foto)}" onerror="this.src='https://via.placeholder.com/60x40?text=No+Img'" style="width:60px;height:40px;object-fit:cover;border-radius:6px;cursor:pointer;" onclick="previewModalGaleri(${g.id})">` : '<span style="color:#d1d5db;">—</span>';
                const ket = g.keterangan ? escHtml(g.keterangan) : '—';
                const desk = g.deskripsi.length > 50 ? escHtml(g.deskripsi.substring(0, 50)) + '...' : escHtml(g.deskripsi);
                return `<tr>
                    <td>${no}</td>
                    <td>${foto}</td>
                    <td><strong>${escHtml(g.judul)}</strong></td>
                    <td>${ket}</td>
                    <td style="max-width:200px;">${desk}</td>
                    <td style="display:flex;gap:6px;flex-wrap:wrap;">
                        <button class="btn-small btn-view" onclick="previewModalGaleri(${g.id})">👁️</button>
                        <button class="btn-small btn-edit" onclick="editGaleri(${g.id})">✏️</button>
                        <button class="btn-small btn-delete" onclick="hapusGaleri(${g.id}, '${escJs(g.judul)}')">🗑️</button>
                    </td>
                </tr>`;
            }).join('');

            // Pagination Galeri
            const totalHal = Math.ceil(semuaGaleri.length / perHalamanGaleri);
            const pag = document.getElementById('galeriPagination');
            if (totalHal <= 1) { pag.innerHTML = ''; return; }
            let html = '';
            for (let p = 1; p <= totalHal; p++) {
                const active = p === halamanGaleri ? 'background:#3b82f6;color:#fff;' : 'background:#f3f4f6;color:#374151;';
                html += `<button onclick="halamanGaleri=${p};renderTabelGaleri()" style="${active}border:none;padding:.4rem .8rem;border-radius:6px;cursor:pointer;font-weight:600;">${p}</button>`;
            }
            pag.innerHTML = html;
        }

        function tampilGaleriNotif(pesan, tipe) {
            const div = document.getElementById('galeriNotif');
            const isOk = tipe === 'ok';
            div.style.cssText = `display:flex;padding:1rem 1.5rem;border-radius:10px;margin-bottom:1.5rem;align-items:center;gap:.75rem;background:${isOk ? '#dbeafe' : '#fee2e2'};color:${isOk ? '#1e40af' : '#991b1b'};border:1px solid ${isOk ? '#bfdbfe' : '#fca5a5'};`;
            div.innerHTML = `<span style="font-size:1.4rem;">${isOk ? '✅' : '❌'}</span> ${pesan}`;
            setTimeout(() => { div.style.display = 'none'; }, 4000);
        }

        function bukaFormGaleri(mode) {
            document.getElementById('formGaleriWrap').style.display = 'block';
            document.getElementById('galeriNotif').style.display = 'none';
            if (mode === 'tambah') {
                document.getElementById('formGaleriTitle').textContent = 'Tambah Foto Baru';
                ...
                document.getElementById('galeriFotoUrl').value = '';
                document.getElementById('galeriFotoFile').value = '';
                hapusGaleriFotoPreview();
                uploadedGaleriPath = '';
            }
            document.getElementById('formGaleriWrap').scrollIntoView({ behavior: 'smooth', block: 'start' });
        }

        function tutupFormGaleri() {
            document.getElementById('formGaleriWrap').style.display = 'none';
            uploadedGaleriPath = '';
        }
        ...
        function previewGaleriFoto(input) {
            if (input.files && input.files[0]) {
                const reader = new FileReader();
                reader.onload = e => {
                    document.getElementById('galeriFotoPreview').src = e.target.result;
                    document.getElementById('galeriFotoPreviewWrap').style.display = 'block';
                    document.getElementById('galeriFotoUrl').value = '';
                };
                reader.readAsDataURL(input.files[0]);
            }
        ...
        function hapusGaleriFotoPreview() {
            document.getElementById('galeriFotoPreview').src = '';
            document.getElementById('galeriFotoPreviewWrap').style.display = 'none';
            document.getElementById('galeriFotoFile').value = '';
        }

        document.getElementById('formGaleri')?.addEventListener('submit', async function (e) {
            e.preventDefault();
            ...
            const fotoFile = document.getElementById('galeriFotoFile').files[0];
            let fotoVal = document.getElementById('galeriFotoUrl').value.trim();

            const btn = document.getElementById('btnSimpanGaleri');
            btn.disabled = true;
            btn.textContent = '⏳ Menyimpan...';
            ...
            try {
                if (fotoFile) {
                    const fd = new FormData();
                    fd.append('foto', fotoFile);
                    const up = await fetch('galery-api.php?action=upload', { method: 'POST', body: fd });
                    const upJ = await up.json();
                    if (!upJ.success) throw new Error('Upload gagal: ' + upJ.message);
                    fotoVal = upJ.filename;
                } else if (uploadedGaleriPath) {
                    fotoVal = uploadedGaleriPath;
                }

                if (!fotoVal && !id) {
                    throw new Error("Foto wajib diunggah atau diisi URL-nya.");
                }

                const action = id ? 'update' : 'create';
                const payload = { judul, foto: fotoVal, keterangan, deskripsi };
                if (id) payload.id = parseInt(id);

                const res = await fetch('galery-api.php?action=' + action, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(payload)
                });
                ...
                tampilGaleriNotif(json.message, 'ok');
                tutupFormGaleri();
                muatGaleri();
            } catch (err) {
                tampilGaleriNotif('Gagal: ' + err.message, 'error');
            } finally {
                btn.disabled = false;
        ...
        async function editGaleri(id) {
            try {
                const res = await fetch('galery-api.php?action=get&id=' + id);
                const json = await res.json();
                if (!json.success) throw new Error(json.message);
                const g = json.data;
                bukaFormGaleri('edit');
                document.getElementById('formGaleriTitle').textContent = 'Edit Foto Galeri';
                ...
                document.getElementById('galeriFotoUrl').value = g.foto || '';
                if (g.foto) {
                    document.getElementById('galeriFotoPreview').src = g.foto;
                    document.getElementById('galeriFotoPreviewWrap').style.display = 'block';
                } else {
                    hapusGaleriFotoPreview();
                }
                uploadedGaleriPath = g.foto || ''; // Tetap gunakan foto lama jika tidak ganti
            } catch (err) {
                tampilGaleriNotif('Gagal memuat data: ' + err.message, 'error');
            }
        }

        async function hapusGaleri(id, judul) {
            if (!confirm('Yakin ingin menghapus foto dari galeri:\n"' + judul + '"?')) return;
            try {
                const res = await fetch('galery-api.php?action=delete', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ id })
                });
                ...
                tampilGaleriNotif('Gagal menghapus: ' + err.message, 'error');
            }
        }

        function previewModalGaleri(id) {
            const g = semuaGaleri.find(x => x.id == id);
            if (!g) return;
            document.getElementById('modalGaleriFoto').src = g.foto || 'https://via.placeholder.com/800x500?text=No+Image';
            document.getElementById('modalGaleriJudul').textContent = g.judul;
            document.getElementById('modalGaleriKeterangan').textContent = g.keterangan ? 'Keterangan: ' + g.keterangan : '';
            document.getElementById('modalGaleriDeskripsi').textContent = g.deskripsi;
            document.getElementById('modalDetailGaleri').style.display = 'block';
        }

        // Attach click listener for Galeri sidebar
        document.addEventListener('DOMContentLoaded', function () {
            document.querySelectorAll('[onclick*="showTab(\'galeri\')"]').forEach(el => {
                el.addEventListener('click', () => setTimeout(muatGaleri, 100));
            });
        ...
        window.onclick = function (event) {
            ...
            if (event.target.id === 'modalDetailGaleri') event.target.style.display = 'none';
        }
    </script>
</body>
